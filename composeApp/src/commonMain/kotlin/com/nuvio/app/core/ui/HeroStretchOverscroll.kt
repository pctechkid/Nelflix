...
private val HeroStretchMax = 200.dp
private const val HeroStretchDragResistance = 0.55f
private const val HeroStretchReleaseAbsorb = 0.35f
private const val HeroStretchFrameToVelocity = 60f
...
class HeroStretchState internal constructor(
    ...
    val nestedScrollConnection: NestedScrollConnection = object : NestedScrollConnection {
        ...
        override fun onPostScroll(
            ...
            if (source == NestedScrollSource.UserInput) {
                val stretch = stretchAnim.value
                val resistance = HeroStretchDragResistance *
                ...
                if (settleJob?.isActive != true) {
                    settle(available.y * HeroStretchFrameToVelocity, HeroStretchBounceSpring)
                }
            }
        ...
        override suspend fun onPreFling(available: Velocity): Velocity {
            ...
            if (settleJob?.isActive != true) {
                settle(available.y * HeroStretchReleaseAbsorb, HeroStretchReleaseSpring)
            }
...
fun rememberHeroStretchState(listState: LazyListState): HeroStretchState {
    ...
    val maxStretchPx = with(LocalDensity.current) { HeroStretchMax.toPx() }
...
@Composable
fun rememberHeroStretchState(scrollState: ScrollState): HeroStretchState {
    val scope = rememberCoroutineScope()
    val maxStretchPx = with(LocalDensity.current) { HeroStretchMax.toPx() }
    return remember(scrollState, maxStretchPx) {
        HeroStretchState(scope, maxStretchPx) { scrollState.value <= 0 }
    }
}
